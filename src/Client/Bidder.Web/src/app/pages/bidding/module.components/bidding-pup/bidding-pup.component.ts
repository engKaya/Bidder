...
export class BiddingPupComponent {
  ...
  submit() {
    if (this.form.invalid) {
      this.toast.openToastError(
        this.translate.instant('FORM_VALIDATIONS.ERROR'),
        this.translate.instant('FORM_VALIDATIONS.FORM_ERRORS')
      );
      return;
    }
    ...
    this.bidService
      ...
      .then((response) => {
        if (response.StatusCode == HttpStatusCode.Ok) {
          this.toast.openToastSuccess(
            this.translate.instant('GENERAL.SUCCESS'),
            this.translate.instant('BID.SUCCESS.AUCTION_CREATED')
          );
